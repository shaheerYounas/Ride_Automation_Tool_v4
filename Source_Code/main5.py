...
# ==========================================
# 5. SOURCE C: GET-E (FIXED LOGIN & ROBUST)
# ==========================================
...
def fetch_gete_rides(driver, processed_list):
    ...
    # --- 1. LOGIN LOGIC (FIXED FOR MUI) ---
    # We wait for the email box by TYPE, not NAME
    try:
        ...
        if is_login_page:
            ...
            # 1. Email Field (Select by type='email')
            ...
            # 2. Password Field (Select by type='password')
            pass_in = driver.find_element(By.CSS_SELECTOR, "input[type='password']")
            pass_in.clear()
            pass_in.send_keys(GETE_PASS)
            
            # 3. Click Sign In (Find button containing 'Sign in')
            try:
            ...
            # 4. Wait for dashboard (URL change)
            print("   -> Waiting for dashboard...")
            WebDriverWait(driver, 25).until(EC.url_contains("rides"))
            time.sleep(5) # Buffer for UI load
            print("   -> Login Successful.")
            
        else:
            print("   -> Already Logged In (No login fields found).")
            
    except Exception as e:
        print(f"   ⚠️ Login process encountered an error (might be already logged in): {e}")

    # --- 2. RESET FILTERS ---
    try:
        # Wait for the dashboard to be ready before clicking reset
        reset_btn = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, "//button[contains(., 'Reset filters')]")))
        driver.execute_script("arguments[0].click();", reset_btn)
        time.sleep(3)
        print("   -> 🔄 Filters reset.")
    except: 
        print("   -> ⚠️ 'Reset filters' button not found. Moving on.")

    # ============================================================
    # PHASE 1: PROCESS "TO CONFIRM"
    # ============================================================
    print("\n   -> 🟢 PHASE 1: Checking 'To confirm'...")
    try:
        # Select "To confirm" tab
        tab_btn = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//button[contains(., 'To confirm')]"))
        )
        ...
        # Confirm Loop
        while True:
            # Find all visible "Confirm" buttons (excluding the tab itself)
            confirm_btns = driver.find_elements(By.XPATH, "//button[contains(., 'Confirm')]")
            confirm_btns = [b for b in confirm_btns if "To confirm" not in b.text and "Confirmed" not in b.text]
            
            if not confirm_btns:
                print("      -> No more rides to confirm.")
                break

            print(f"      -> Found {len(confirm_btns)} rides to confirm.")
        ...
        print(f"   ⚠️ 'To confirm' phase skipped/empty. Error: {e}")

    # ============================================================
    # PHASE 2: SWITCH TO 'CONFIRMED'
    # ============================================================
    print("\n   -> 🔵 PHASE 2: Scraping 'Confirmed'...")
    try:
        tab_btn = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//button[contains(., 'Confirmed')]"))
        )
        driver.execute_script("arguments[0].click();", tab_btn)
        time.sleep(5) 
    except Exception as e:
        print(f"   ❌ Critical: Could not switch to 'Confirmed' tab. Error: {e}")
        return []
    ...
    while True:
        ...
        for r_idx in row_indices:
            try:
                ...
                driver.get(GETE_LOGIN_URL) 
                time.sleep(3)
                try: driver.find_element(By.XPATH, "//button[contains(., 'Confirmed')]").click()
                except: pass
                time.sleep(3)
