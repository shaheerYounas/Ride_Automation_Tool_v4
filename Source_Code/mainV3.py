...
# ==========================================
# 5. SOURCE C: GET-E (SMART SCRAPER 2.0)
# ==========================================
...
def fetch_gete_rides(driver, processed_list):
    ...
    # RESET FILTERS
    try:
        reset_btn = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, "//button[contains(., 'Reset filters')] | //button[.//div[contains(text(), 'Reset filters')]]")))
        ...
        print(f"   -> ⚠️ Could not find Reset Filters: {e}")
    ...
    try:
        ...
        for idx in row_ids_to_process:
            try:
                ...
                # 1. DATE SCRAPER (PRECISE SELECTOR)
                full_dt_str = ""
                try:
                    # "Wed, 10 Dec 2025 at 14:10 PM"
                    # Locate the specific span near "Pickup date and time"
                    date_ele = driver.find_element(By.XPATH, "//*[contains(text(), 'Pickup date and time')]/following-sibling::span | //*[contains(text(), 'Pickup date and time')]/..//span[contains(@class, 'css-1q4c11b')]")
                    raw_date_text = date_ele.text.strip()
                    
                    # Clean and Parse
                    clean_dt = raw_date_text.replace("at ", "").replace(" PM", "").replace(" AM", "")
                    dt_obj = datetime.strptime(clean_dt, "%a, %d %b %Y %H:%M")
                    full_dt_str = dt_obj.strftime("%Y-%m-%dT%H:%M:00")
                except:
                    # FALLBACK: Use Regex on body text for safety
                    try:
                        date_matches = re.findall(r'[A-Za-z]{3}, \d{1,2} [A-Za-z]{3} \d{4} at \d{1,2}:\d{2} [AP]M', page_text)
                        if date_matches:
                            clean_dt = date_matches[0].replace("at ", "").replace(" PM", "").replace(" AM", "")
                            dt_obj = datetime.strptime(clean_dt, "%a, %d %b %Y %H:%M")
                            full_dt_str = dt_obj.strftime("%Y-%m-%dT%H:%M:00")
                        else:
                ...
                try:
                    if "Passenger information" in page_text:
                        parts = page_text.split("Passenger information")
                        if len(parts) > 1:
                            info_block = parts[1].split("\n")
                            for line in info_block[:5]:
                                if len(line) > 3 and not "Driver" in line and not "Vehicle" in line:
                                    name = line
                                    break
                except: pass
                ...
                # 3. OTHER DATA
                phone = ""
                phone_match = re.search(r'\+\d{8,15}', page_text)
                if phone_match: phone = phone_match.group(0)

                flight = ""
                flight_matches = re.findall(r'\(([A-Z0-9]{2,8})\)', page_text)
                if flight_matches: flight = flight_matches[0]

                pax_count = "1"
                pax_match = re.search(r'Passengers\s+(\d+)', page_text)
                if pax_match: pax_count = pax_match.group(1)

                pickup = "Unknown"
                dropoff = "Unknown"
                if "Ride information" in page_text:
                    ride_block = page_text.split("Ride information")[1].split("Passenger information")[0]
                    lines = [l.strip() for l in ride_block.split("\n") if len(l.strip()) > 5]
                    if len(lines) >= 3:
                        pickup = lines[1]
                        dropoff = lines[-1]
                ...
                elif "Van" in page_text or "People" in page_text: vehicle = "Minivan"
