...
CONTRACTOR_ID_GETE = "FIX_ME" # <--- ENTER THE ID FOR GET-E HERE
CONTRACTOR_TEXT_GETE = "GET"  # Fallback text if ID unknown
    ...
    for page in range(0, 1): 
        ...
        for r in results:
            if r['journeyCode'] not in processed_list:
                ...
                # --- ADDRESS CLEANUP (Prefer original if avail) ---
                # This fixes the "full hotel name" issue by taking the cleaner name if available
                ...
                try:
                    ...
                    for row in rows:
                        try:
                            ...
                            # --- FLIGHT NUMBER FIX ---
                            # Extract ONLY flight code (e.g. EW9773) ignoring time
                            ...
                            for line in pax_lines:
                                ...
                                if "Executive" in line: vehicle = "Business"  # NEW RULE
        ...
        for idx in row_ids_to_process:
            try:
                ...
                try:
                    ...
                    full_dt_str = datetime.now().strftime("%Y-%m-%d 12:00:00") 
...
def process_single_order(driver, ride, wait):
    try:
        ...
        if ride['source'] == "KOI":
            ...
            if CONTRACTOR_ID_GETE != "FIX_ME": current_contractor = CONTRACTOR_ID_GETE
            else: current_contractor = "TEXT_MODE_GETE"
        ...
        try:
            ...
            if current_contractor == "TEXT_MODE":
                ...
                try: contractor_dropdown.select_by_visible_text(CONTRACTOR_TEXT_GETE)
                except: pass
            else:
